...
def retrieve_context(
        ...
        hybrid_search_weight: float = 0.7  # Weight for vector search (1-weight for keyword)
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        try:
            ...
            entity_results: QueryResult = entities_collection.query(
                ...
                hybrid=True,
                alpha=hybrid_search_weight  # Adjust balance between vector and keyword search
            )
        except Exception as e:
    ...
    if retrieved_context["top_entities"]:
        ...
        CALL apoc.path.subgraphNodes(start_node, {{
            ...
            relationshipFilter: "CONTAINS>|CONTAINS_METHOD>|CALLS>|IMPORTS>|INHERITS_FROM>|IMPLEMENTS>"
        }}) YIELD node
        ...
        if node_id_results and node_id_results[0].get("node_ids"):
            ...
            WITH n1.id AS source_id,
                 ...
                 CASE type(r)
                   ...
                   WHEN 'CONTAINS_METHOD' THEN 4
                   WHEN 'IMPLEMENTS' THEN 3
                   WHEN 'INHERITS_FROM' THEN 3
                   WHEN 'IMPORTS' THEN 2
                   ELSE 1
